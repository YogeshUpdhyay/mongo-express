...
import authFuncs from './routes/auth.js';
...
const router = async function (config) {
  ...
  const authenticationFunctions = await authFuncs(config);
  console.log(config.useOAuth);
  if (config.useOAuth) {
    appRouter.get('/login', authenticationFunctions.loginController);
    appRouter.get('/oauth/callback', authenticationFunctions.authorizedCallback);
  }
